...
def test_for_dataset(model, dataloader, num_classes, log_file, cuda, opt):
    ...
    for label in range(num_classes):
        ...
        for annotation in all_annotations:
            if annotation[0].shape[0] == 0:
                row_str = row_str + '\n'
            for i in range(annotation[0].shape[0]):
                if i + 1 != annotation[0].shape[0]:
                    row_str = row_str + ','.join(map(str, annotation[0][i].tolist())) + '_'
                else:
                    row_str = row_str + ','.join(map(str, annotation[0][i].tolist())) + '\n'
        with open('test_log/gt_and_pred/' + str(label) + '_' + str(opt.conf_thres) + '_gt.txt', 'w') as outf:
            outf.write(row_str)
    ...
    for label in range(num_classes):
        ...
        for detection in all_detections:
            if detection[0].shape[0] == 0:
                row_str = row_str + '\n'
            for i in range(detection[0].shape[0]):
                if i + 1 != detection[0].shape[0]:
                    row_str = row_str + ','.join(map(str, detection[0][i].tolist())) + '_'
                else:
                    row_str = row_str + ','.join(map(str, detection[0][i].tolist())) + '\n'
        with open('test_log/gt_and_pred/' + str(label) + '_' + str(opt.conf_thres) + '_pred.txt', 'w') as outf:
            outf.write(row_str)
...
def main():
    ...
    opt.model_config_path = "../../training/config/yolov3.cfg"
    opt.data_config_path = "../../training/config/chestlesionbright.data"
    opt.weights_path = "../../training/saved/20190415_112547_annotation_after_v2_middle/checkpointsbright/101_evalloss=9.436.weightss"
