...
def test_for_dataset(model, dataloader, num_classes, log_file, cuda, opt):
    ...
    :param model: models.eval()をしてある学習済みモデル
    ...
    for batch_i, (_, imgs, targets) in enumerate(tqdm.tqdm(dataloader, desc="Detecting objects")):
        ...
        with torch.no_grad():
            outputs = model(imgs) # outputsは、x, y, w, h, conf(obj/cls), conf(cls)が入り、x,yなどは絶対サイズが入る。
            outputs = non_max_suppression(outputs, NUMCLASSES, conf_thres=opt.conf_thres, nms_thres=opt.nms_thres)
            # outputs=batch_sizeの長さのリスト。リストの各値にNMS後の結果が格納されていて、結果には、x, y, w, h, conf(obj/class), conf(class), ラベルが入る。
    ...
    for label in range(num_classes):
        ...
        for i in tqdm.tqdm(range(len(all_annotations)), desc="Computing AP for class:{}".format(label)):
            ...
            num_annotations += annotations.shape[0] # 一つのdetectionの数
            ...
            for *bbox, score in detections: #{
                ...
                if max_overlap >= opt.iou_thres and assigned_annotation not in detected_annotations: # and score >= XXXはどうだろう？？
        ...
        print("# of annotations=",num_annotations)
        recall = true_positives / num_annotations # recallの累積
        ...
        # mean precision and recall of models per label
...
def main():
    ...
    opt.conf_thres = 0.9 # 可変。conf(obj/cls)を対象とした閾値。
    opt.iou_thres = 0.1 # 固定 TP/FPを判定する時の、GTとdetection resultの判定のための閾値
    opt.nms_thres = 0.1 # 固定 NMSを実施するときの、detectionの結果の4番目(conf[obj/class])をマージする閾値
    opt.img_size = 512
    opt.batch_size = 5
    num_classes = 1
    ...
    test_path = opt.image_folder + 'brighttestdata.txt'
