...
data_config = parse_data_config(opt.data_config_path) #trainデータの画像へのパスたちなどが記されたtxtファイルへの $
train_path = data_config["train"] #trainキーには学習に使う画像へのパスたち（これを読めば各行が画像パスに対応する）$
valid_path = data_config["valid"] #評価画像
num_classes = int(data_config["classes"]) #クラス

# Get dataloader
if opt.context: #{
    dataloader = DataLoader(ImageTensorFolder(valid_path, img_size=opt.img_size),
...
for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
    ...
    if detections is not None:
        ...
        for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
            ...
            color = cmap(0) #bbox_colors[int(np.where(unique_labels == int(cls_pred))[0])]
